...
echo "###############################################"
echo "$(date) Uploading undercloud scripts $UNDERCLOUD (root)"
$SCP -r scripts/undercloud-install.sh scripts/overcloud-{images-$OPENSTACK_VERSION,introspection,deploy,post}.sh scripts/{opensink,follow-events.py} $ENVIRONMENT/{environment,undercloud.conf,instackenv.json,network-environment.yaml,nic-configs} stack@$UNDERCLOUDIP:
...
if [ "x$SSL_ENABLE" != "x" ]
 then
  ...
  $SCP scripts/undercloud-ssl.sh $ENVIRONMENT/undercloud.pem stack@$UNDERCLOUDIP:
...
if [ "x$INTROSPECTION_POST_SCRIPT" != "x" ]
 then
  ...
  $SCP $ENVIRONMENT/$INTROSPECTION_POST_SCRIPT stack@$UNDERCLOUDIP:
...
if [ "x$SSL_ENABLE" != "x" ]
 then
  echo "###############################################"
  echo "$(date) Uploading SSL configuration $UNDERCLOUD (stack)"
  $SCP scripts/overcloud-ssl.sh $ENVIRONMENT/{cloudname,enable-tls,inject-trust-anchor}.yaml $ENVIRONMENT/overcloud-cacert.pem stack@$UNDERCLOUDIP:
